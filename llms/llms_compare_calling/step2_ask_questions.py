...
# Usage:
#     python step2_ask_questions.py -a
"""
...
def write_results_to_json_file(bag):
    ...
    clean_str = title_to_alphanum(bag.model_ollama)
    fname = (f"{bag.dir_out}/answers_{clean_str}.json")
...
def questions_part1(bag):
    ...
    print(f" === Chat Vs Instruct style:                  ======================\n")
    print(f" === Chat: prompt message vs user message")
    print(f" === VS ")
    print(f" === Instruct: prompt message vs prompt + system message")
    print(f" === Questions generator: {bag.questions_data['llm_question_generator']} \n")
    print(f" === Models in test: {bag.llms}")

#---------------------------------------------------------------
def questions_part2_ask(bag):
    """ part2 - called from create_questions() in the loop, asks questions """
    # ---------------------------------------------
    init_chat_system_ollama_llm(bag)
    print("Ask question through system message")
    resp = bag.llm_ollama.chat(model=bag.model_ollama, messages=bag._ollama_system_message)
    bag.asnwer_llm_system_message = resp['message']['content']
    # ---------------------------------------------
    init_chat_user_ollama_llm(bag)
    print("Ask question through user (non system) message")
    resp = bag.llm_ollama.chat(model=bag.model_ollama, messages=bag._ollama_user_message)
    bag.asnwer_llm_user_message = resp['message']['content']
    # ---------------------------------------------
    init_instruct_prompt_llm(bag)
    print("Ask question using instruct prompt style")
    ...
    # ---------------------------------------------
    print("Ask question using instruct prompt and system style")
    resp = bag.llm_ollama.generate(model=bag.model_ollama, prompt=bag.question, system=bag.instructions)
    bag.asnwer_llm_instruct_prompt_and_system = resp['response']
...
def questions_part3_print(bag):
    ...
    print("\nAnswer from instruct prompt style llm:")
    print(f"\n{bag.asnwer_llm_instruct_prompt}")
    print("\nAnswer from instruct prompt and system style llm:")
    print(f"\n{bag.asnwer_llm_instruct_prompt_and_system}")
...
def create_questions(bag):
    ...
    for q_idx in range(len(bag.questions_data['questions'])):
        ...
        answer_info = {
            ...
            "answer_instruct_prompt_and_system_llm" : bag.asnwer_llm_instruct_prompt
        }
